...
def get_device_data(request: Request, device_id: str = None):
    client = MongoClient(MONGO_URI)
    db = client.iot_data
    collection = db.sensor_readings

    # Fetch unique Device_ID values for the dropdown
    device_ids = collection.distinct("Device_ID")

    # Fetch data based on the selected device_id or fetch the latest 10 records
    if device_id:
        devices = list(collection.find({"Device_ID": int(device_id)}, {"_id": 0}))
    else:
        devices = list(collection.find({}, {"_id": 0}).sort("_id", -1).limit(10))

    # Log the fetched data for debugging
    print("Fetched devices:", devices)
    print("Available Device IDs:", device_ids)
